...
cp -r "$SCRIPT_DIR/magpi_packages" "$BOOTFS/"
echo "  -> magpi_packages/ copied"

# -----------------------------------------------------------------------------
# 2. Copy magpi_revd_setup.sh
# -----------------------------------------------------------------------------
echo "[2/6] Copying magpi_revd_setup.sh..."
...
for f in magpi-network.service eth0-static.nmconnection magpi_revd_setup.sh; do
  if [ -f "$BOOTFS/$f" ]; then
    echo "  OK  $f"
  else
    echo "  MISSING  $f"
...
echo "  3. SSH in:     ssh bird@192.168.1.1"
