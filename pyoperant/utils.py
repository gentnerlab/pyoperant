...
def run_state_machine(
    start_in="pre", error_state=None, error_callback=None, **state_functions
):
...
class Trial(Event):
    ...
    def __init__(self, index=None, type_="normal", class_=None, *args, **kwargs):
...
def check_cmdline_params(parameters, cmd_line):
    ...
    if not (
        ...
        == int(
            ...
            .encode("ascii", "ignore")
            .translate(allchars, nodigs)
        )
    ):
        print("box number doesn't match config and command line")
        return False
    if not (
        "subj" not in cmd_line
        or int(cmd_line["subj"].encode("ascii", "ignore").translate(allchars, nodigs))
        == int(
...
def concat_wav(input_file_list, output_filename="concat.wav"):
    ...
    with closing(wave.open(output_filename, "wb")) as output:
        for input_filename, isi in input_file_list:
            ...
            if isi > 0.0:
                isi_frames = "".join(
                    [struct.pack("h", fr) for fr in [0] * int(fs * isi)]
                )
