...
_ZOG_MAP = {
    1: (
        ...
        2,
        8,
    ),  # box_id:(subdevice,in_dev,in_chan,out_dev,out_chan)
...
class ZogPanel(panels.BasePanel):
    ...
    def __init__(self, id=None, *args, **kwargs):
        ...
        self.interfaces["comedi"] = comedi_.ComediInterface(
            device_name=_ZOG_MAP[self.id][0]
        )
        self.interfaces["pyaudio"] = ZogAudioInterface(
            device_name=(dev_name_fmt % self.id)
        )
        ...
        for in_chan in [ii + _ZOG_MAP[self.id][2] for ii in range(4)]:
            self.inputs.append(
                hwio.BooleanInput(
                    ...
                    params={"subdevice": _ZOG_MAP[self.id][1], "channel": in_chan},
                )
            )
        for out_chan in [ii + _ZOG_MAP[self.id][4] for ii in range(8)]:
            self.outputs.append(
                hwio.BooleanOutput(
                    interface=self.interfaces["comedi"],
                    params={"subdevice": _ZOG_MAP[self.id][3], "channel": out_chan},
                )
            )
...
class ZogCuePanel(ZogPanel):
    ...
    def __init__(self, id=None):
        ...
        self.cue = components.RGBLight(
            red=self.outputs[7], green=self.outputs[5], blue=self.outputs[6], name="cue"
        )
