...
class PiPanel(panels.BasePanel):
    ...
    def __init__(self, id=None, *args, **kwargs):
        ...
        for in_chan in INPUTS:
            self.inputs.append(
                hwio.BooleanInput(
                    interface=self.interfaces["raspi_gpio_"],
        ...
        # Standard Peckports
        self.left = components.PeckPort(
            IR=self.inputs[1], LED=self.pwm_outputs[4], name="l", inverted=False
        )
        self.center = components.PeckPort(
            IR=self.inputs[2], LED=self.pwm_outputs[5], name="c", inverted=False
        )
        self.right = components.PeckPort(
            IR=self.inputs[3], LED=self.pwm_outputs[6], name="r", inverted=False
        )

        # Hopper
        self.hopper = components.Hopper(
            IR=self.inputs[0], solenoid=self.outputs[0], inverted=True
        )
...
BEHAVIORS = ["pyoperant.behavior", "glab_behaviors"]
