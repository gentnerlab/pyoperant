...
class Shaper(object):
    ...
    def run_shape(self, start_state="block1"):
        ...
        self.log.warning(
            "Shaping procedure complete.  Remember to disable shaping in your config file"
        )
    ...
    def _hopper_block(self, block_num):
        ...
        def temp():
            ...
            utils.run_state_machine(
                ...
                flash_mid=self._flash_poll(
                    self.panel.center, 5, "reward", "pre_reward"
                ),
        ...
        def temp():
            if not self.responded_block:
                ...
                if elapsed_time > revert_timeout:
                    self.log.warning(
                        "No response in block %d, reverting to block %d.  Time: %s"
                        % (
                            self.recent_state,
                            self.recent_state - 1,
    ...
    def _poll(
        self, component, duration, next_state, reward_state=None, poll_state=None
    ):
        if poll_state == None:
    ...
    def _flash_poll(self, component, duration, next_state, reward_state=None):
        return self._poll(
            ...
            next_state,
            reward_state,
            poll_state=self._flashing_main,
        )

    def _light_poll(self, component, duration, next_state, reward_state=None):
        return self._poll(
            component, duration, next_state, reward_state, poll_state=self._light_main
        )
    ...
    def _response_2ac_block(self, block_num, reps=100, revert_timeout=10800):
        ...
        def temp():
            ...
            utils.run_state_machine(
                ...
                poll_right=self._flash_poll(
                    self.panel.right, 10, "check_right", "pre_reward"
                ),
                check_left=self._check_block("poll_left", reps, revert_timeout),
                poll_left=self._flash_poll(
                    self.panel.left, 10, "check_left", "pre_reward"
                ),
                pre_reward=self._pre_reward("reward"),
    ...
    def _response_3ac_block(self, block_num, reps=100, revert_timeout=10800):
        ...
        def temp():
            ...
            utils.run_state_machine(
                ...
                poll_center=self._flash_poll(
                    self.panel.center, 10, "check_center", "pre_reward"
                ),
                check_left=self._check_block("poll_left", reps, revert_timeout),
                poll_left=self._flash_poll(
                    self.panel.left, 10, "check_left", "pre_reward"
...
class Shaper3ACMatching(Shaper3AC):
    ...
    def _response_3ac_matching_audio_block(
        self, block_num, reps=150, revert_timeout=10800
    ):
        def temp():
            self.recent_state = block_num
