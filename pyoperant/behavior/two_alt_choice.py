...
class TwoAltChoiceExp(base.BaseExp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "reinforcement" in self.parameters.keys():
            ...
            if reinforcement["schedule"] == "variable_ratio":
                self.reinf_sched = reinf.VariableRatioSchedule(
                    ratio=reinforcement["ratio"]
                )
            elif reinforcement["schedule"] == "fixed_ratio":
                self.reinf_sched = reinf.FixedRatioSchedule(
                    ratio=reinforcement["ratio"]
                )
            elif reinforcement["schedule"] == "percent_reinf":
                self.reinf_sched = reinf.PercentReinforcement(
                    prob=reinforcement["prob"]
                )
            else:
            ...
            self.parameters["block_design"] = {
                "blocks": {
                    "default": {
                        "queue": "random",
                        "conditions": [
                            {"class": k} for k in self.parameters["classes"].keys()
                        ],
                    }
                },
                "order": ["default"],
    ...
    def session_main(self):
        ...
        if self.session_q is None:
            self.log.info(
                "Next sessions: %s" % self.parameters["block_design"]["order"]
            )
            self.session_q = queues.block_queue(
                self.parameters["block_design"]["order"]
            )
        ...
        if self.trial_q is None:
            for sn_cond in self.session_q:
                ...
                if q_type == "random":
                    ...
                    dbl_staircases = [
                        queues.DoubleStaircaseReinforced(stims)
                        for stims in blk["stim_lists"]
                    ]
                    self.trial_q = queues.MixedAdaptiveQueue.load(
                        os.path.join(
                            self.parameters["experiment_path"], "persistentQ.pkl"
                        ),
                        dbl_staircases,
                    )
                try:
    ...
    def new_trial(self, conditions=None):
        ...
        if self.do_correction:
            ...
            trial = utils.Trial(
                type_="correction", index=index, class_=self.trials[-1].class_
            )
        ...
        self.log.debug(
            "trial %i: %s, %s"
            % (self.this_trial.index, self.this_trial.type_, self.this_trial.class_)
        )
    ...
    def trial_pre(self):
        ...
        self.log.debug(
            "number of open file descriptors: %d" % (utils.get_num_open_fds())
        )

        self.this_trial = self.trials[-1]
        min_wait = self.this_trial.stimulus_event.duration
        max_wait = (
            self.this_trial.stimulus_event.duration + self.parameters["response_win"]
        )
    ...
    def trial_post(self):
        ...
        self.this_trial.duration = (
            dt.datetime.now() - self.this_trial.time
        ).total_seconds()
        self.analyze_trial()
    ...
    def stimulus_main(self):
        ...
        if "cue" in self.this_trial.annotations:
            ...
            cue_event = utils.Event(
                time=cue_time, duration=cue_dur, label="cue", name=cue
            )
    ...
    def consequence_main(self):
        # correct trial
        if (
            self.this_trial.response
            == self.parameters["classes"][self.this_trial.class_]["component"]
        ):
    ...
    def update_adaptive_queue(self, presented=True):
        if self.this_trial.type_ == "normal" and isinstance(
            self.trial_q, queues.AdaptiveBase
        ):
            if presented:
                self.trial_q.update(
                    self.this_trial.correct, self.this_trial.response == "none"
                )
            else:
    ...
    def reward_main(self):
        ...
        ## ways to abstract this
            ...
            utils.wait(
                self.parameters["classes"][self.this_trial.class_]["reward_value"]
            )
            # self.panel.reset()
